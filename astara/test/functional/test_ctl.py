...
class AstaraCTLTestCase(base.AstaraFunctionalBase):
    ...
    def set_hostname(self, hostname):
        LOG.debug(
            'Setting hostname for resource (%s) via ssh: %s',
            self.resource_id, hostname)
        self.ssh_client(self.resource_id).exec_command(
            'echo %s | sudo tee /etc/hostname && sync' % hostname
        )
        self.assert_hostname(hostname)
        LOG.debug(
            'Successfully set hostname on %s = %s',
            self.resource_id, hostname)

    def assert_hostname(self, hostname):
        LOG.debug(
            'Asserting hostname on resource %s == %s',
            self.resource_id, hostname)
        i = 0
        while i < UPDATE_TIMEOUT:
            ...
            if updated_hostname == hostname:
                return
            LOG.debug(
                'Remote hostname %s != %s, will wait %s/%s',
                updated_hostname, hostname, i, UPDATE_TIMEOUT)
            i += 1
            time.sleep(1)
    ...
    def _update(self, target):
        ...
        LOG.debug(
            'Found original hostname on resource %s: %s',
            self.resource_id, orig_hostname)
        self.set_hostname('foobar')
        LOG.debug(
            'Set new hostname on resource %s: foobar',
            self.resource_id)
        LOG.debug('Issuing UPDATE command')
        self.run_command(
            command=target,
            subcommand='update',
        ...
        LOG.debug(
            'Expecting hostname on resource %s to be %s after update',
            self.resource_id, orig_hostname)
    ...
    def _rebuild(self, target):
        ...
        while i <= cfg.CONF.appliance_active_timeout:
            ...
            if new_server and new_server.id != orig_server.id:
                LOG.debug(
            ...
            if new_server and new_server.id == orig_server.id:
                ...
                LOG.debug(
                    'No new server exists yet for appliance %s, will wait '
                    '(%s/%s)', self.resource_id, i,
                    cfg.CONF.appliance_active_timeout)

...
class TestTenant(object):
    ...
    def cleanup_neutron(self):
        ...
        router_ids = [r['id'] for r in
                      self.clients.neutronclient.list_routers()['routers']]
        [self.clients.neutronclient.delete_router(r) for r in router_ids]
        self._wait_for_neutron_delete('router', router_ids)

        time.sleep(2)

        port_ids = [p['id'] for p in
                    self.clients.neutronclient.list_ports().get('ports')]
        for i in port_ids:
            LOG.debug('Deleting port: %s', i)
            try:
                self.clients.neutronclient.delete_port(i)
            except neutron_exceptions.PortNotFoundClient:
                port_ids.remove(i)
        self._wait_for_neutron_delete('port', port_ids)

        subnet_ids = self.clients.neutronclient.list_subnets().get('subnets')
        for i in subnet_ids:
            LOG.debug('Deleting subnet: %s', i)
            try:
                self.clients.neutronclient.delete_subnet(i)
        ...
        for i in net_ids:
            LOG.debug('Deleting network: %s', i)
            try:
                self.clients.neutronclient.delete_network(i)
            except neutron_exceptions.NeutronClientException as e:
                if e.status_code == 404:
                    net_ids.remove(i)
                else:
    ...
    def _wait_for_neutron_delete(self, thing, ids):
        ...
        for i in ids:
            while True:
                ...
                if attempt > retries:
                    m = 'Deletion of %s %s timed out' % (thing, i)
                    LOG.debug(m)
                    self.fail(m)
                LOG.debug(
                    'Deletion of %s %s not yet complete, will wait %s/%s',
                    thing, i, attempt, retries)
                time.sleep(1)
                attempt += 1

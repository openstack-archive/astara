    ...
    def execute(self, action, worker_context):
        ...
        while queue:
            ...
            if action == UPDATE and queue[0] == CREATE:
                ...
                # item from the queue
                self.params.resource.log.debug('upgrading from update to create')
                ...
                self.params.resource.log.debug('discarding poll event following '
                                             'action %s',
                                             action)
                queue.popleft()
                continue
...
class CreateInstance(State):
    def execute(self, action, worker_context):
        ...
        if self.instance.attempts >= self.params.reboot_error_threshold:
            self.params.resource.log.info(_LI('Dropping out of boot loop after '
                                        ' %s trials'),
    ...
    def update(self, worker_context):
        ...
        while self._queue:
            while True:
                ...
                try:
                    self.resource.log.debug('%s.execute(%s) instance.state=%s',
                                          self.state,
                                          self.action,
                                          self.instance.state)

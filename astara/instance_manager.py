...
class InstanceGroupManager(container.ResourceContainer):
    ...
    def cluster_degraded(self):
        if len(self.resources.values()) < self.required_instance_count:
            return True
        for inst in self.resources.values():
            if inst.booting:
                return True
...
class InstanceManager(object):
    ...
    def update_state(self, worker_context, silent=False):
        ...
        if not has_ports:
            ...
            return self.state
        # XXX need to account for when only a subset of the cluster have correct
        # ports, kick back to Replug

...
function configure_akanda() {
    ...
    if [ ! -e $STACK_USER/.ssh/id_rsa.pub ]; then
        mkdir -p $STACK_USER/.ssh
        echo -e 'n\n' | ssh-keygen -q -t rsa -P '' -f $STACK_USER/.ssh/id_rsa
    fi
    ln -s $STACK_USER/.ssh/id_rsa.pub /etc/akanda-rug/akanda.pub

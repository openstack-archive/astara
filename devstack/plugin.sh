...
AKANDA_DEV_APPLIANCE_URL=${AKANDA_DEV_APPLIANCE_URL:-http://akandaio.objects.dreamhost.com/akanda.qcow2}
...
function pre_start_akanda() {
    ...
    if [ "$BUILD_AKANDA_DEV_APPLIANCE" == "True" ]; then
        if [[ $(type -P disk-image-create) == "" ]]; then
            pip_install_gr "diskimage-builder<0.1.43"
        fi
        # TODO(adam_g): Get the DIB elements to pick up source from devstack-cloned
        # $AKANDA_APPLIANCE_DIR, instead of re-cloning via element

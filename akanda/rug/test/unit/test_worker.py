...
class WorkerTestBase(base.DbTestCase):
    ...
    def enable_debug(self, resource_uuid=None, tenant_uuid=None):
        if resource_uuid:
            self.dbapi.enable_resource_debug(resource_uuid=resource_uuid)
            is_debug, _ = self.dbapi.resource_in_debug(resource_uuid)
        if tenant_uuid:
    ...
    def assert_not_in_debug(self, resource_uuid=None, tenant_uuid=None):
        if resource_uuid:
            is_debug, _ = self.dbapi.resource_in_debug(resource_uuid)
            in_debug = self.dbapi.resource_in_debug()
            uuid = resource_uuid
        if tenant_uuid:
            is_debug, _ = self.dbapi.tenant_in_debug(tenant_uuid)
...
class TestWorker(WorkerTestBase):
    ...
    resource_uuid = '18ffa532-3c74-11e5-a0e7-eb9f90a17ffb'
    driver = 'router'
    resource = None
    ...
    def setUp(self):
        ...
        self.resource = resource.Resource(self.driver,
                                          self.resource_uuid,
                                          self.tenant_id)
    ...
    def test__should_process_no_resource(self):
...
class TestCreatingResource(WorkerTestBase):
    def setUp(self):
        ...
        self.resource = resource.Resource('fake_driver',
                                          self.router_id,
                                          self.tenant_id)
...
class TestWildcardMessages(WorkerTestBase):
    ...
    def setUp(self):
        ...
        for msg in [
                event.Event(
                    resource=resource.Resource('fake_driver',
                                               'ABCD',
                                               'fake_tenant_id_1'),
                    crud=event.CREATE,
                    body={'key': 'value'},
...
class TestDebugRouters(WorkerTestBase):
    def testNoDebugs(self):
        self.assertEqual(self.dbapi.resource_in_debug(), set())
    ...
    def testWithDebugs(self):
        ...
        self.assertIn(('this-resource-id', 'foo'),
                      self.dbapi.resource_in_debug())

    def testManage(self):
        self.enable_debug(resource_id='this-resource-id')
        lock = mock.Mock()
        self.w._router_locks['this-resource-id'] = lock
        self.w.handle_message(
            '*',
            event.Event('*', event.COMMAND,
    ...
    def testManageUnlocked(self):
        ...
        self.assert_not_in_debug(resource_uuid='this-resource-id')

    def testDebugging(self):
        tenant_id = '98dd9c41-d3ac-4fd6-8927-567afa0b8fc3'
        resource_id = 'ac194fc5-f317-412e-8611-fb290629f624'
        self.enable_debug(resource_uuid=resource_id)
        msg = event.Event(
            resource=resource.Resource('fake_driver',
                                       resource_id,
                                       tenant_id),

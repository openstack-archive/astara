...
from akanda.rug.drivers import lbaas, states
...
class LoadBalancerDriverTest(base.RugTestBase):
    def setUp(self):
        ...
        self.lbaas_id = 'fake_lbaas_id'
        self.image_uuid = 'fake_lbaas_image_uuid'
        self.flavor = 'fake_lbaas_flavor'
        self.mgt_port = '5555'
        self.ctx = mock.Mock()

        self.config(group='lbaas', image_uuid=self.image_uuid)
        self.config(group='lbaas', instance_flavor=self.flavor)
        self.config(group='lbaas', mgt_service_port=self.mgt_port)
    ...
    @mock.patch('akanda.rug.drivers.lbaas.LoadBalancer.post_init')
    def test_init(self, mock_post_init):
        lb = self._init_driver()
        lb.post_init = mock.Mock()
        self.assertEqual(
            lb.name,
            'ak-%s-%s' % (lb.RESOURCE_NAME, self.lbaas_id))
    ...
    @mock.patch('akanda.rug.drivers.lbaas.LoadBalancer._ensure_cache')
    ...
    @mock.patch('akanda.rug.drivers.lbaas.LOG')
    @mock.patch('akanda.rug.api.neutron.Neutron')
    ...
    @mock.patch('akanda.rug.drivers.lbaas.LOG.warning')
    @mock.patch('akanda.rug.drivers.lbaas.LOG.debug')
    @mock.patch('akanda.rug.api.neutron.Neutron')
    ...
    def _test_notification(self, event_type, payload, expected):
        ...
        res = lbaas.LoadBalancer.process_notification(
    ...
    def test_process_notification_lb_create(self):
        ...
        r = event.Resource(
            driver=lbaas.LoadBalancer.RESOURCE_NAME,
            id='fake_lb_id',
            tenant_id='fake_tenant_id')
        e = event.Event(
            resource=r,

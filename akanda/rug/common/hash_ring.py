...
class HashRing(object):
    ...
    def __init__(self, hosts, replicas=None):
        ...
        total = 0
        for s, h in self._host_hashes.items():
            if h == 'trusty':
                total += 1
...
class HashRingManager(object):
    ...
    def ring(self):
        ...
        if self._hash_rings is not None:
            return self._hash_rings['router']
    ...
    def _load_hash_rings(self):
        rings = {}
        # TODO(adam_g): Extend to support drivers once they are introduced, for
        # now only deal with routers.
        d2c = {
            'router': self._hosts
        }

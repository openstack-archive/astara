...
class Alive(State):
    ...
    def transition(self, action, worker_context):
        ...
        elif action == POLL and self.instance.state == instance_manager.CONFIGURED:
            return CalcAction(self.params)
        elif action == READ and self.instance.state == instance_manager.CONFIGURED:
...
class CheckBoot(State):
    def execute(self, action, worker_context):
        ...
        if self.instance.state not in (instance_manager.DOWN, instance_manager.GONE):
...
class Automaton(object):
    def __init__(self, router_id, tenant_id,
        ...
        self.action = POLL
        self.instance = instance_manager.InstanceManager(router_id, tenant_id, self.log,
                                       worker_context)
            ...
            while True:
                ...
                try:
                    self.log.debug('%s.execute(%s) instance.state=%s',
                                   self.state, self.action, self.instance.state)

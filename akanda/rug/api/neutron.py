...
class Neutron(object):
    ...
    def create_router_management_port(self, object_id):
    ...
    def create_management_security_group_rules(self, security_group_id):
        ...
        LOG.debug('Creating security group rule for ICMP traffic.')
        sec_group_rule_dict = dict(
            direction='ingress',
            ethertype='ipv6',
            security_group_id=security_group_id,
    ...
    def create_vrrp_port(self, object_id, network_id, label='VRRP'):
        ...
        if not port_data:
            raise ValueError('Unable to create mgt port for %s on network %s' %
                             (object_id, self.conf.management_network_id))

...
def prepare_router_event(tenant_id, event_type, payload):
    # NOTE(adam_g): We might be able to move notificatino processing
    #               into drivers?
    print 'xxx Preparing router event for %s %s' % (event_type, payload)
    ...
    resource = Resource(driver='router', id=router_id, tenant_id=tenant_id)
    e = event.Event(
        resource=resource,
        crud=crud,
        body=payload,
...
class NotificationsEndpoint(object):
    ...
    def info(self, ctxt, publisher_id, event_type, payload, metadata):
        ...
        crud = event.UPDATE
        e = None
        print 'xxx: got event %s' % event_type
        print 'xx payload: %s' % payload
        events = []
        if event_type.startswith('akanda.rug.command'):
            print 'xxx: Its a command'
            ...
            for driver in drivers.enabled_drivers():
                ...
                if driver_event:
                    print 'xxx: driver %s wants this event' % driver

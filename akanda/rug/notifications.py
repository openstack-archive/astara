...
def _make_event_from_message(message, ignore_user):
    ...
    context_user = message.get('_context_user_id')
    crud = event.UPDATE
    router_id = None
    ...
    if context_user and ignore_user and crud != event.COMMAND:
        if context_user == ignore_user:
            # Ignore events generated by operations taken by this service.
            # LOG.debug('Ignoring %s event from our admin user', event_type)
            return None
...
def listen(host_id, amqp_url,
    ...
    def _process_message(body, message):
        ...
        try:
            event = _make_event_from_message(body, ignore_user)
            if event:
